...
__attribute__((sdx_kernel("dft", 0))) void dft(STYPE &real_sample, STYPE &imag_sample,STYPE &real_op, STYPE &imag_op);
# 3 "dft.cpp" 2
...
#pragma HLS INTERFACE axis depth=50 port=imag_op
 ...
 float re_sample[1024];
 float im_sample[1024];
...
#pragma HLS pipeline II=12
 ...
 if(0==n){
  ...
  buff_re += im_sample_n * s;
     buff_im += re_sample_n * s;
  buff_im += im_sample_n_neg * c;
  if(1024 -1 == n){
   pkt Xre;
   pkt Xim;
   Xre.data = buff_re;
