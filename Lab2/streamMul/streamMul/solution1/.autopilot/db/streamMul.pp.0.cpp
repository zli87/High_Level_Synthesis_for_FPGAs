...
# 242 "C:/Xilinx/Vitis_HLS/2020.2/common/technology/autopilot\\ap_axi_sdata.h" 2
namespace hls {
...
__attribute__((sdx_kernel("smul", 0))) void smul(hls::stream<pkt > &INPUT, hls::stream<pkt > &OUTPUT, unsigned int length);
# 2 "streamMul.cpp" 2

__attribute__((sdx_kernel("smul", 0))) void smul(hls::stream<pkt > &INPUT, hls::stream<pkt > &OUTPUT, unsigned int length){
#pragma HLS TOP name=smul
# 3 "streamMul.cpp"
...
#pragma HLS INTERFACE axis depth=50 port=INPUT
 ap_int<32> buff[50];
 VITIS_LOOP_9_1: for(unsigned int i=0; i<length; i++){
  pkt tmp;
  INPUT.read(tmp);
  buff[i] = tmp.data * 2;
 }
 VITIS_LOOP_14_2: for(unsigned int i=0; i<length; i++){
  pkt tmp;
  tmp.data = buff[i];
  OUTPUT.write(tmp);
 }
