...
typedef struct {
    ...
    XAxi4_sqrt_uio_map maps[ MAX_UIO_MAPS ];
} XAxi4_sqrt_uio_info;

/***************** Variable Definitions **************************************/
static XAxi4_sqrt_uio_info uio_info;
...
static int uio_info_read_name(XAxi4_sqrt_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAxi4_sqrt_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAxi4_sqrt_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAxi4_sqrt_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAxi4_sqrt_Initialize(XAxi4_sqrt *InstancePtr, const char* InstanceName) {
	XAxi4_sqrt_uio_info *InfoPtr = &uio_info;
